...
type RawVM interface {
	...
	ConfigHTTPService(serviceName, ip, port string) error
}
...
var (
	...
	vmZone        = flag.String("vm_zone", "us-west1-c", "The zone in which the VM resides")
	clusterName   = flag.String("cluster_name", "prow", "The name of the istio cluster that the VM extends")
	clusterZone   = flag.String("cluster_zone", "us-west1-a", "The zone of the istio cluster that the VM extends")
...
func NewGCPRawVM(name, namespace string) *GCPRawVM {
	return &GCPRawVM{
		Name:           name,
...
// ConfigHTTPService configures an http service running on the VM, which
// is used to test if vm properly extends the istio service mesh
func (vm *GCPRawVM) ConfigHTTPService(serviceName, port string) error {
	internalIP, err := vm.GetInternalIP()
	if err != nil {
		return err
	}
...
func (vm *GCPRawVM) provision() error {
	...
	// _, err := u.Shell(`gcloud compute firewall-rules create allow-external \
	// 	--allow tcp:22,tcp:80,tcp:443,tcp:9411,udp:5228,icmp \
...
func replaceKVInYamlThenKubectlApply(yamlPath string, kv map[string]string) error {
	bytes, err := ioutil.ReadFile(mashExpansionYaml)
	if err != nil {
		return err
	}
	yaml := string(bytes)
